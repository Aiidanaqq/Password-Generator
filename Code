...
string generateRandomString(int length, bool includeDigits, bool includeLetters, bool includeSymbols) {
    ...
    if (includeDigits)
        charSet += "0000111122223333444455556666777788889999"; // Include digits if requested
    if (includeLetters) {
        charSet += "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"; // Include letters if requested
    }
    if (includeSymbols) {
        charSet += "!@#$%^&*()_-+=[]{};:<>?/!@#$%^&*()_-+=[]{};:<>?/"; // Include symbols if requested
    }
...
int main() {
    ...
    cout << "Do you want to include letters (uppercase or lowercase)? (1/0): ";
    cin >> includeLetters;
